...
class DictSearchController extends Cubit<DictSearchState> {
  DictSearchController([int length = 5])
    : super(DictSearchState.initial(length));
  ...
  void setLength(int length) {
    ...
    emit(
      state.copyWith(
        length: length,
        current: state.current.length > length
            ? state.current.substring(0, length)
            : state.current,
      ),
    );
    getSuggestions();
  ...
  void getSuggestions() {
    final suggestions = dictionary()
        .getSuggestions(state.current, state.length)
        .toList();

    emit(
      state.copyWith(
        suggestions: suggestions,
        valid: suggestions.contains(state.current),
      ),
    );
  }
}
...
class DictSearchState {
  ...
  factory DictSearchState.initial([int length = 5]) =>
      DictSearchState(current: '', length: length);
