...
class AuthHandler {
  static Future<Response> register(Request request) async {
    try {
      ...
      if (!isValidUsername(username)) {
        return HttpUtils.buildErrorResponse('invalid_username');
      }
      ...
      final uResult = await userStore().write(user);
      if (!uResult.ok) return HttpUtils.buildErrorResponse(Errors.unknown);
      final aResult = await authStore().write(authData);
  ...
  static Future<Response> login(Request request) async {
    try {
      ...
      if (a.object!.password == null) {
        return HttpUtils.buildErrorResponse('no_password');
      }
      if (!checkpw(password, a.object!.password!)) {
        return HttpUtils.buildErrorResponse('wrong_password');
      }
      return HttpUtils.buildResponse(
        data: {'user': user.toMap()},
        tokenData: issueToken(user.id),
  ...
  static Future<Response> changePassword(Request request) async {
    try {
      ...
      if (!aResult.ok) return HttpUtils.buildErrorResponse(Errors.unknown);
      if (!checkpw(oldPass, aResult.object!.password!)) {
        return HttpUtils.buildErrorResponse(Errors.unauthorised);
      }
