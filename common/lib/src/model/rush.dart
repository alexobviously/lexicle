...
class Rush {
  ...
  int? get endTime => config.timeLimit != null
      ? startTime + config.timeLimit! + timeAdjustment
      : null;
  ...
  int get numRows =>
      completed.fold<int>(0, (a, b) => a + b.numRows) + current.numRows;
  ...
  Rush withNewWord(Game g) =>
      copyWith(completed: [...completed, current], current: g);
  Rush timeAdjusted(int adjustment) =>
      copyWith(timeAdjustment: timeAdjustment + adjustment);

  String toEmojis() {
    String output = completed.map((e) => e.toEmojis()).toList().join('\n');
    String current = this.current.toEmojis();
    if (current.isNotEmpty) output = '$output\n$current';
    return output;
  }
}
